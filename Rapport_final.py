...
# 
# $$E[R_a] = RFR + \beta_a (R_m - RFR)$$
# 
# - $E[R_a]$ : the expected return of the asset a that we want to calculate<br>
# - $RFR$ : the risk free rate<br>
# - $\beta_a$ : the beta of asset a<br>
# - $R_m$ : the average profitability of the market<br>
# 
...
# The next term of the sum of the CAPM brings up two other notions: the $\beta_a$ and the **profitability of the market $R_m$**.
# 
# The **beta** $\beta_a$ is the measure of the correlation between the profitability of the **asset a** and the **market m**, that is to say the trend of evolution of our asset compared to that of the market.
...
# 
# The general idea of ​​the model is similar to a generalized additive model. The "prophet's equation" corresponds to trend, seasonality and holidays. This is given by,<br><br>
# **$$y[t]=g[t]+s[t]+h[t]+e[t]$$**
# - $y[t]$ is the forecast
...
# 
# Prophet is used here with the **piecewise linear model** to predict data (but it can also work with the logistic growth model if specified).<br><br>
# 
# $$y = \beta_0 + \beta_1*x \quad if \quad x <= c$$
# 
# $$y = \beta_0 - \beta_2*c + (\beta_1 + \beta_2)*x \quad if \quad x>c $$ 
# 
...
# <img src='markowitz_ml.png'>


# We see that **dynamic strategy** with rebalancing the weights of each stock is **more efficient** (except for CAPM).


# # Overall comparison against S&P 500 benchmark
...
# 
# Regarding the Sharpe ratio indicator, the strategy of Markowitz with historical mean has the best risk reward.
# Moreover the performance exceeds that of the investment at the risk-free rate.
# 
